...
const Navbar = () => {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-[#F7F7F5]/90 backdrop-blur-md border-b border-black/5 py-3 md:py-4' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="container mx-auto px-4 sm:px-6 md:px-12 flex justify-between items-center">
        <Link to="/" className="flex items-center h-9 w-32 sm:h-10 sm:w-40 relative">
        ...
        <button className="md:hidden text-[#1F2937] p-2 -mr-2" aria-label="Toggle navigation" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="md:hidden absolute top-full left-0 w-full max-h-[calc(100vh-72px)] overflow-y-auto bg-[#F7F7F5] border-b border-black/5 flex flex-col p-6 shadow-xl"
          >
